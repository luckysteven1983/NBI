...
select top 1 convert(char(10),endTime,111)+' '+convert(char(10),endTime,108) from CM_syncFlowStatus where flow='LTEFUL_AMOS' and flag=0 order by endTime desc
go
EOF`
...
for NETYPE in AntennaFunction EnbFunction EpRpDynS1uEnb EpRpStaS1MmeEnb EthernetPort EutranCellTdd EutranRelationTdd GsmRelation InventoryUnitAccessory InventoryUnitPack InventoryUnitRack InventoryUnitRru InventoryUnitShelf ManagedElement SctpAssoc UtranRelationTdd  GGSN SGW PGW SGSN MME PCRF
do
   echo "\n---------------------------------------------------------" >> $CompareDetailFile
   echo "TestCase: TYPE $NETYPE comparation" >> $CompareDetailFile
        ...
        case $NETYPE in
        SGSN|GGSN)
            cmfileName="CMCC-"$NETYPE"-NRM-V4.0.2-"$pmfileTimestamp.xml.gz
            ;;
        PGW|SGW|PCRF|MME)
            cmfileName="CMCC-"$NETYPE"-NRM-V1.2.0-"$pmfileTimestamp.xml.gz
            ;;
        *)
            cmfileName="CMCC-ENB-NRM-V2.1.0-"$NETYPE"-"$cmfileTimestamp.xml.gz
            ;;
        esac
