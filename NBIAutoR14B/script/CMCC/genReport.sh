...
then
   ...
   COREINVResult=`grep "Failed" $cmsyncLog |grep COREINV`
   if [ $? -eq 0 ]
   then
      echo "   CM Sync with flow=COREINV Failed, check $cmsyncLog and CMMapping.log for details"  >> $reportFile
      exit 1
   else
      echo "   `grep 'COREINV Successfully' $cmsyncLog`" >> $reportFile
   fi
 
   FULLAMOS=`grep "Failed" $cmsyncLog |grep FULL_AMOS`
   if [ $? -eq 0 ]
   then
      echo "   CM Sync with flow=FULL_AMOS Failed, check $cmsyncLog and CMMapping.log for details"  >> $reportFile 
      exit 1
   else
      echo "   `grep 'FULL_AMOS Successfully' $cmsyncLog `" >> $reportFile
   fi
   
   WPSyncResult=`grep "Failed" $cmsyncLog |grep ENODEB_WP`
   if [ $? -eq 0 ]
   then
      echo "   CM Sync with flow=ENODEB_WP Failed, check $cmsyncLog and CMMapping.log for details"  >> $reportFile
      exit 1
   else
      echo "   `grep 'ENODEB_WP Successfully' $cmsyncLog `" >> $reportFile
   fi
...
writeCheckAllModulesStatusResult()
{
cmsyncLog=$1
reportFile=$2

echo "4. Check All Modules Status result" >> $reportFile

if [ -f $cmsyncLog ]
then
   cat $cmsyncLog  >> $reportFile 
else
   echo $cmsyncLog not exist
fi
   echo "\n" >> $reportFile
...
echo "5. CM Comparation Result" >> $reportFile
printf "%30s %10s    %-30s\n" "Type" "Result" "Details" >> $reportFile

for MOC in AntennaFunction EnbFunction EpRpDynS1uEnb EpRpStaS1MmeEnb EthernetPort EutranCellTdd EutranRelationTdd GsmRelation InventoryUnitAccessory InventoryUnitPack InventoryUnitRack InventoryUnitRru InventoryUnitShelf ManagedElement SctpAssoc UtranRelationTdd GGSN SGW PGW SGSN MME PCRF HLR HSS
do
...
echo "6. PM Comparation Result" >> $reportFile
printf "%30s %10s    %-30s\n" "Type" "Result" "Details" >> $reportFile

for MOC in EpRpDynS1uEnb EthernetPort EutranCellTdd EutranRelationTdd ManagedElement SctpAssoc GGSN SGW PGW SGSN MME PCRF HLR HSS
do
  failFlag="false"
  passFlag="true"
  i=0
...
AllModulesStatusLog=$FinalResultLoc/checkAllModulesStatus.log
writeCheckAllModulesStatusResult $AllModulesStatusLog $FinalReport
