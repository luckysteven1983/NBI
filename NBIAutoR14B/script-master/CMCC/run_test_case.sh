...
# Author: eyujche
# Date: 2013-11-15
# Description: This script used for NBI CM/PM test case execution, it invoke a stax xml and tracking the progress
...
case $step in
Step1)
   output="Check all modules status"
   ;;
Step2)
   output="Generate CM Files"
   ;;
Step3)
   output="Compare CM North File"
   ;;
Step4)
   output="Compare PM North File"
   ;;
...
scriptName=$CIGITHome/script-master/CMCC/CMPMTestCase.xml

userlogName=$ID"_TestCaseLog"
...
then
   for N in 1 2 3 4
   do
      if checkExecuteStatus Step$N "Start" $userlogName; then
         msgbox Step$N "Start"
         if checkExecuteStatus Step$N "Finished" $userlogName; then
